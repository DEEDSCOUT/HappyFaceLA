...
def validate_phase1c_input(
    ...
    """Non-mutating Phase 1C candidate-input intake gate (Phase 1B.5A/5B/5C-R).
    ...
    The candidate must be a COMPLETE DRAFT commercial governance payload: all
    six record families must be non-empty, exactly one DRAFT ReleaseRecord
    shell must be present, every projected output channel must have exactly
    one corresponding DRAFT activation row, and every used source_model must
    be addressable against the frozen workbook destinations with all required
    governance fields having exactly one column mapping to the authorized
    destination tab.  Rule categories must come from the canonical
