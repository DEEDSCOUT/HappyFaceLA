...
function pickTier(lead: NormalizedLead): Exclude<PricingTier, "custom"> {
    ...
    const hours = firstInt(lead.event_length) ?? 0;
    let tier: Exclude<PricingTier, "custom"> = "standard";
    if (guests <= 12) tier = "basic";
    else if (guests > 25) tier = "premium";
    if (hours >= 3) tier = "premium";
    if (hours > 0 && hours <= 1 && guests <= 15) tier = "basic";
    return tier;
}
...
export function recommendPricing(lead: NormalizedLead, score: LeadScore): PricingRecommendation {
    ...
    if (capacity_review) {
        notes.push(
            `Guest count ${lead.guest_count} > ${CAPACITY_SINGLE_ARTIST}: recommend a 2nd artist (capacity quote).`,
        );
    }

    // 3+ services or large-capacity events go to custom quote.
    if (serviceCount >= 3 || (capacity_review && (lead.guest_count ?? 0) > 50)) {
        notes.push("3+ services or large capacity — build a custom quote.");
        return {
...
export function buildReplyDraft(
    ...
    const capacity =
        pricing.capacity_review
            ? ` With your guest count I'd bring an extra artist so no one waits in line.`
            : "";
...
function buildAlert(
    card: Pick<LeadCard, "lead" | "score" | "pricing" | "reply_draft">,
