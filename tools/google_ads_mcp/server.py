...
if __package__ in (None, ""):
    ...
    from tools.google_ads_mcp import auth, config as cfg_mod, diagnostics, gaql, mutations, schemas  # type: ignore
    from tools.google_ads_mcp.google_ads_client import build_client, row_to_dict  # type: ignore
else:
    from . import auth, config as cfg_mod, diagnostics, gaql, mutations, schemas
    from .google_ads_client import build_client, row_to_dict
...
def _resolve_campaign(client, name_or_id: str) -> dict[str, Any]:
    ...
    cid = CFG.customer_id
    where = f"campaign.id = {int(name_or_id)}" if name_or_id.isdigit() else f"campaign.name = '{name_or_id}'"
    q = f"""
...
@mcp.tool()
def google_ads_campaign_diagnostics(campaign_name: str = "Kids Party Face Painting") -> dict:
    ...
    match = [c for c in camps if name_lc in (c.get("campaign", {}).get("name") or "").lower()]
    if not match:
        return {"error": f"No campaign matched '{campaign_name}'.", "candidates": [c.get("campaign", {}).get("name") for c in camps]}
    camp = match[0]
    camp_id = camp["campaign"]["id"]
    asset_groups = gaql.stream_to_dicts(client, cid, f"""
    ...
    """)
    signals = gaql.stream_to_dicts(client, cid, f"""
    ...
    """)
    locs = gaql.stream_to_dicts(client, cid, f"""
    ...
    customer_goals = gaql.stream_to_dicts(client, cid, diagnostics.Q_CUSTOMER_CONV_GOALS)
    campaign_goals = gaql.stream_to_dicts(client, cid, diagnostics.Q_CAMPAIGN_CONV_GOALS)
    ...
    return {
        ...
        "campaign_conversion_goals": [g for g in campaign_goals if g.get("campaign", {}).get("id") == camp_id],
...
def google_ads_conversion_actions() -> list[dict]:
    return gaql.stream_to_dicts(_client(), CFG.customer_id, diagnostics.Q_CONVERSION_ACTIONS)
...
def google_ads_conversion_goals() -> dict:
    client = _client()
    return {
        "customer": gaql.stream_to_dicts(client, CFG.customer_id, diagnostics.Q_CUSTOMER_CONV_GOALS),
        "campaign": gaql.stream_to_dicts(client, CFG.customer_id, diagnostics.Q_CAMPAIGN_CONV_GOALS),
    }


@mcp.tool()
def google_ads_pmax_asset_groups() -> list[dict]:
    return gaql.stream_to_dicts(_client(), CFG.customer_id, diagnostics.Q_ASSET_GROUP_ASSETS)
...
def google_ads_keywords_and_negatives() -> dict:
    client = _client()
    cid = CFG.customer_id
    kw = gaql.stream_to_dicts(client, cid, """
    ...
    """)
    neg = gaql.stream_to_dicts(client, cid, """
...
def google_ads_change_history() -> list[dict]:
    try:
        return gaql.stream_to_dicts(_client(), CFG.customer_id, diagnostics.Q_CHANGE_EVENTS)
...
def google_ads_update_campaign_status(payload: dict) -> dict:
    ...
    with mutations.with_safety(
        CFG, tool="google_ads_update_campaign_status", reason=inp.reason,
        risk_level="high" if inp.status == "REMOVED" else "medium",
        validate_only=inp.validate_only, approval_token=inp.approval_token,
        resources_touched=[resource_name], before_data=before,
    ) as plan:
        ...
        plan["operations"] = [{
            "service": "CampaignService", "operation": "update",
            "resource_name": resource_name,
            "fields_changed": {"status": inp.status},
            "old_values": {"status": before["campaign"].get("status")},
            "new_values": {"status": inp.status},
        }]
        resp = mutations.run_mutate(client, "CampaignService", CFG.customer_id, [op], inp.validate_only)
        plan["after"] = {"validate_only": inp.validate_only, "results": [str(r) for r in resp.results]}
        return {"plan": plan["plan"], "plan_path": plan["plan_path"], "validate_only": inp.validate_only}
...
def google_ads_update_campaign_budget(payload: dict) -> dict:
    ...
    mutations.check_budget_delta(CFG, old_micros, new_micros, inp.force_large_budget_change)
    with mutations.with_safety(
        CFG, tool="google_ads_update_campaign_budget", reason=inp.reason,
        risk_level="high", validate_only=inp.validate_only, approval_token=inp.approval_token,
        resources_touched=[budget_rn], before_data=camp,
    ) as plan:
        ...
        plan["operations"] = [{
            "service": "CampaignBudgetService", "operation": "update",
            "resource_name": budget_rn,
            "fields_changed": {"amount_micros": new_micros},
            "old_values": {"amount_micros": old_micros, "amount_usd": mutations.micros_to_usd(old_micros)},
            "new_values": {"amount_micros": new_micros, "amount_usd": inp.new_daily_budget_usd},
        }]
        resp = mutations.run_mutate(client, "CampaignBudgetService", CFG.customer_id, [op], inp.validate_only)
        plan["after"] = {"validate_only": inp.validate_only, "results": [str(r) for r in resp.results]}
        return {"plan": plan["plan"], "plan_path": plan["plan_path"], "validate_only": inp.validate_only}
...
def google_ads_update_bid_strategy(payload: dict) -> dict:
    ...
    with mutations.with_safety(
        CFG, tool="google_ads_update_bid_strategy", reason=inp.reason,
        risk_level="high", validate_only=inp.validate_only, approval_token=inp.approval_token,
        resources_touched=[resource_name], before_data=camp,
    ) as plan:
        op = client.get_type("CampaignOperation")
        op.update.resource_name = resource_name
        paths: list[str] = []
        if inp.strategy == "MAXIMIZE_CONVERSIONS":
            ...
            if inp.target_cpa_usd:
                op.update.maximize_conversions.target_cpa_micros = mutations.budget_usd_to_micros(inp.target_cpa_usd)
            ...
            op.update.target_cpa.target_cpa_micros = mutations.budget_usd_to_micros(inp.target_cpa_usd or 0)
        ...
        plan["operations"] = [{
            ...
            "old_values": {"strategy": camp["campaign"].get("bidding_strategy_type")},
            "new_values": {"strategy": inp.strategy, "target_cpa_usd": inp.target_cpa_usd, "target_roas": inp.target_roas},
        }]
        resp = mutations.run_mutate(client, "CampaignService", CFG.customer_id, [op], inp.validate_only)
        plan["after"] = {"validate_only": inp.validate_only, "results": [str(r) for r in resp.results]}
        return {"plan": plan["plan"], "plan_path": plan["plan_path"], "validate_only": inp.validate_only}
...
def google_ads_update_conversion_action(payload: dict) -> dict:
    ...
    before_rows = gaql.stream_to_dicts(client, CFG.customer_id, f"""
    ...
    with mutations.with_safety(
        CFG, tool="google_ads_update_conversion_action", reason=inp.reason,
        risk_level="high", validate_only=inp.validate_only, approval_token=inp.approval_token,
        resources_touched=[rn], before_data=before,
    ) as plan:
        op = client.get_type("ConversionActionOperation")
        op.update.resource_name = rn
        paths: list[str] = []
        if inp.name is not None:
            op.update.name = inp.name; paths.append("name")
        if inp.category is not None:
            op.update.category = client.enums.ConversionActionCategoryEnum[inp.category]; paths.append("category")
        if inp.status is not None:
            op.update.status = client.enums.ConversionActionStatusEnum[inp.status]; paths.append("status")
        ...
        if inp.primary_for_goal is not None:
            op.update.primary_for_goal = inp.primary_for_goal; paths.append("primary_for_goal")
        if inp.counting_type is not None:
            op.update.counting_type = client.enums.ConversionActionCountingTypeEnum[inp.counting_type]
        ...
        client.copy_from(op.update_mask, {"paths": paths})
        plan["operations"] = [{
            "service": "ConversionActionService", "operation": "update",
            "resource_name": rn,
            "fields_changed": {p: getattr(inp, p.split('.')[0], None) for p in paths},
            "old_values": before["conversion_action"],
            "new_values": inp.model_dump(exclude_none=True),
        }]
        resp = mutations.run_mutate(client, "ConversionActionService", CFG.customer_id, [op], inp.validate_only)
        plan["after"] = {"validate_only": inp.validate_only, "results": [str(r) for r in resp.results]}
        return {"plan": plan["plan"], "plan_path": plan["plan_path"], "validate_only": inp.validate_only}
...
def google_ads_update_customer_conversion_goal(payload: dict) -> dict:
    ...
    before = gaql.stream_to_dicts(client, CFG.customer_id, diagnostics.Q_CUSTOMER_CONV_GOALS)
    with mutations.with_safety(
        CFG, tool="google_ads_update_customer_conversion_goal", reason=inp.reason,
        risk_level="high", validate_only=inp.validate_only, approval_token=inp.approval_token,
        resources_touched=[rn], before_data=before,
    ) as plan:
        ...
        plan["operations"] = [{
            "service": "CustomerConversionGoalService", "operation": "update",
            "resource_name": rn,
            "fields_changed": {"biddable": inp.biddable},
            "old_values": {"category": inp.category, "origin": inp.origin},
            "new_values": {"biddable": inp.biddable},
        }]
        resp = mutations.run_mutate(client, "CustomerConversionGoalService", CFG.customer_id, [op], inp.validate_only)
        plan["after"] = {"validate_only": inp.validate_only, "results": [str(r) for r in resp.results]}
        return {"plan": plan["plan"], "plan_path": plan["plan_path"], "validate_only": inp.validate_only}
...
def google_ads_update_campaign_conversion_goals(payload: dict) -> dict:
    ...
    camp = _resolve_campaign(client, inp.campaign_name_or_id)
    camp_id = camp["campaign"]["id"]
    before = gaql.stream_to_dicts(client, CFG.customer_id, diagnostics.Q_CAMPAIGN_CONV_GOALS)
    with mutations.with_safety(
        CFG, tool="google_ads_update_campaign_conversion_goals", reason=inp.reason,
        risk_level="high", validate_only=inp.validate_only, approval_token=inp.approval_token,
        resources_touched=[f"customers/{CFG.customer_id}/campaigns/{camp_id}"], before_data=before,
    ) as plan:
        ops = []
        for g in inp.goals:
            ...
            ops.append(op)
        plan["operations"] = [{
            "service": "CampaignConversionGoalService", "operation": "update",
            "resource_name": op.update.resource_name,
            "fields_changed": {"biddable": op.update.biddable},
            "old_values": {},
            "new_values": g,
        } for op, g in zip(ops, inp.goals)]
        resp = mutations.run_mutate(client, "CampaignConversionGoalService", CFG.customer_id, ops, inp.validate_only)
...
def google_ads_add_campaign_negative_keywords(payload: dict) -> dict:
    ...
    if (camp["campaign"].get("advertising_channel_type") or "").upper() == "PERFORMANCE_MAX":
        raise ValueError(
    ...
    with mutations.with_safety(
        CFG, tool="google_ads_add_campaign_negative_keywords", reason=inp.reason,
        risk_level="low", validate_only=inp.validate_only, approval_token=inp.approval_token,
        resources_touched=[camp_rn], before_data=camp,
    ) as plan:
        ops = []
        for kw in inp.keywords:
            ...
            op.create.keyword.match_type = client.enums.KeywordMatchTypeEnum[inp.match_type]
            ops.append(op)
        plan["operations"] = [{
            "service": "CampaignCriterionService", "operation": "create",
            "resource_name": f"{camp_rn}/criteria/<new>",
            "fields_changed": {"keyword.text": kw, "keyword.match_type": inp.match_type, "negative": True},
            "old_values": {},
            "new_values": {"keyword": kw, "match_type": inp.match_type},
        } for kw in inp.keywords]
        resp = mutations.run_mutate(client, "CampaignCriterionService", CFG.customer_id, ops, inp.validate_only)
        plan["after"] = {"validate_only": inp.validate_only, "count": len(resp.results)}
        return {"plan": plan["plan"], "plan_path": plan["plan_path"], "validate_only": inp.validate_only}
...
def google_ads_create_search_campaign(payload: dict) -> dict:
    ...
    client = _client()
    with mutations.with_safety(
        CFG, tool="google_ads_create_search_campaign", reason=inp.reason,
        risk_level="high", validate_only=inp.validate_only, approval_token=inp.approval_token,
        resources_touched=["<new campaign>"], before_data={"name": inp.name},
    ) as plan:
        # 1) budget
        ...
        b_resp = mutations.run_mutate(client, "CampaignBudgetService", CFG.customer_id, [b_op], inp.validate_only)
        budget_rn = b_resp.results[0].resource_name if not inp.validate_only else "<validate-only>"
        ...
        c_op.create.advertising_channel_type = client.enums.AdvertisingChannelTypeEnum.SEARCH
        ...
        c_resp = mutations.run_mutate(client, "CampaignService", CFG.customer_id, [c_op], inp.validate_only)

        plan["operations"] = [
            {"service": "CampaignBudgetService", "operation": "create",
             "resource_name": budget_rn, "fields_changed": {"amount_usd": inp.daily_budget_usd},
             "old_values": {}, "new_values": {"name": b_op.create.name, "amount_usd": inp.daily_budget_usd}},
            {"service": "CampaignService", "operation": "create",
             "resource_name": "<new campaign>",
             "fields_changed": {"name": inp.name, "status": "PAUSED"},
             "old_values": {},
             "new_values": {"name": inp.name, "channel": "SEARCH", "bidding": inp.bidding, "final_url": inp.final_url}},
        ]
        plan["after"] = {
...
def google_ads_create_or_update_search_keywords(payload: dict) -> dict:
    ...
    if any((k.get("match_type") or "").upper() == "BROAD" for k in inp.keywords) and "BROAD_OK" not in inp.reason.upper():
    ...
    with mutations.with_safety(
        CFG, tool="google_ads_create_or_update_search_keywords", reason=inp.reason,
        risk_level="medium", validate_only=inp.validate_only, approval_token=inp.approval_token,
        resources_touched=[ag_rn], before_data={"ad_group": ag_rn, "keywords": inp.keywords},
    ) as plan:
        ops = []
        for k in inp.keywords:
            ...
            op.create.keyword.match_type = client.enums.KeywordMatchTypeEnum[k.get("match_type", "PHRASE")]
            ops.append(op)
        plan["operations"] = [{
            "service": "AdGroupCriterionService", "operation": "create",
            "resource_name": f"{ag_rn}/criteria/<new>",
            "fields_changed": k, "old_values": {}, "new_values": k,
        } for k in inp.keywords]
        resp = mutations.run_mutate(client, "AdGroupCriterionService", CFG.customer_id, ops, inp.validate_only)
        plan["after"] = {"validate_only": inp.validate_only, "count": len(resp.results)}
        return {"plan": plan["plan"], "plan_path": plan["plan_path"], "validate_only": inp.validate_only}
...
def google_ads_update_pmax_assets(payload: dict) -> dict:
    ...
    with mutations.with_safety(
        CFG, tool="google_ads_update_pmax_assets", reason=inp.reason,
        risk_level="medium", validate_only=inp.validate_only, approval_token=inp.approval_token,
        resources_touched=[ag_rn], before_data={"asset_group": ag_rn},
    ) as plan:
        ...
        if text_specs:
            ...
            for text, _ in text_specs:
                ...
                a_ops.append(aop)
            a_resp = mutations.run_mutate(client, "AssetService", CFG.customer_id, a_ops, inp.validate_only)
            for (text, ftype), r in zip(text_specs, a_resp.results):
                new_asset_rns.append((r.resource_name if not inp.validate_only else "<validate-only>", ftype))
        ...
        for ytid in inp.youtube_video_ids:
            ...
            vresp = mutations.run_mutate(client, "AssetService", CFG.customer_id, [vop], inp.validate_only)
            new_asset_rns.append((vresp.results[0].resource_name if not inp.validate_only else "<validate-only>", "YOUTUBE_VIDEO"))
        ...
        for rn, ftype in new_asset_rns:
            ...
            link_ops.append(lop)
        if link_ops:
            l_resp = mutations.run_mutate(client, "AssetGroupAssetService", CFG.customer_id, link_ops, inp.validate_only)
        else:
            l_resp = None
        plan["operations"] = [{
            "service": "AssetGroupAssetService", "operation": "create",
            "resource_name": ag_rn, "fields_changed": {"field_type": ftype, "asset": rn},
            "old_values": {}, "new_values": {"asset": rn, "field_type": ftype},
        } for rn, ftype in new_asset_rns]
        plan["after"] = {
            "validate_only": inp.validate_only,
            "linked": len(link_ops),
...
def google_ads_update_locations(payload: dict) -> dict:
    ...
    camp_id = camp["campaign"]["id"]
    camp_rn = f"customers/{CFG.customer_id}/campaigns/{camp_id}"
    before = gaql.stream_to_dicts(client, CFG.customer_id, f"""
        SELECT campaign_criterion.resource_name, campaign_criterion.location.geo_target_constant
        FROM campaign_criterion WHERE campaign.id = {int(camp_id)} AND campaign_criterion.type = 'LOCATION'
    """)
    with mutations.with_safety(
        CFG, tool="google_ads_update_locations", reason=inp.reason,
        risk_level="medium", validate_only=inp.validate_only, approval_token=inp.approval_token,
        resources_touched=[camp_rn], before_data=before,
    ) as plan:
        ops = []
        ...
        rn_by_gtc = {b["campaign_criterion"]["location"]["geo_target_constant"]: b["campaign_criterion"]["resource_name"] for b in before}
        ...
        plan["operations"] = (
            [{"service": "CampaignCriterionService", "operation": "create",
              "resource_name": f"{camp_rn}/criteria/<new>", "fields_changed": {"location.geo_target_constant": gtc},
              "old_values": {}, "new_values": {"geo_target_constant": gtc}} for gtc in inp.add_geo_target_constants]
            + [{"service": "CampaignCriterionService", "operation": "remove",
                "resource_name": rn_by_gtc.get(gtc, "<missing>"),
                "fields_changed": {}, "old_values": {"geo_target_constant": gtc}, "new_values": {}}
               for gtc in inp.remove_geo_target_constants]
        )
        resp = mutations.run_mutate(client, "CampaignCriterionService", CFG.customer_id, ops, inp.validate_only)
        plan["after"] = {"validate_only": inp.validate_only, "count": len(resp.results)}
...
def google_ads_upload_image_assets(payload: dict) -> dict:
    ...
    client = _client()
    with mutations.with_safety(
        CFG, tool="google_ads_upload_image_assets", reason=inp.reason,
        risk_level="medium", validate_only=inp.validate_only, approval_token=inp.approval_token,
        resources_touched=["<new asset>"], before_data={"source": str(p)},
    ) as plan:
        ...
        plan["operations"] = [{
            "service": "AssetService", "operation": "create",
            "resource_name": "<new asset>",
            "fields_changed": {"name": inp.asset_name, "size_bytes": len(data)},
            "old_values": {}, "new_values": {"source": str(p), "name": inp.asset_name},
        }]
        resp = mutations.run_mutate(client, "AssetService", CFG.customer_id, [op], inp.validate_only)
        plan["after"] = {"validate_only": inp.validate_only, "results": [str(r) for r in resp.results]}
        return {"plan": plan["plan"], "plan_path": plan["plan_path"], "validate_only": inp.validate_only}
...
def google_ads_apply_recommendation(payload: dict) -> dict:
    ...
    if inp.validate_only:
        return {"validate_only": True, "note": "RecommendationService.apply_recommendation has no validate_only mode; "
    ...
    client = _client()
    with mutations.with_safety(
        CFG, tool="google_ads_apply_recommendation", reason=inp.reason,
        risk_level="critical", validate_only=False, approval_token=inp.approval_token,
        ...
        plan["operations"] = [{
            "service": "RecommendationService", "operation": "apply",
        ...
        return {"plan": plan["plan"], "plan_path": plan["plan_path"], "validate_only": False}
