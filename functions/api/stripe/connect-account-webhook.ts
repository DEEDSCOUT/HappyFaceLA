...
export const onRequest = async (context: {
  ...
  try {
    const config = getPayoutRuntimeConfig(context.env, "account-webhook");
    if (
      !context.env.PAYOUTS_D1 ||
      !config.stripeSecretKey ||
      !config.stripeWebhookSecret
    ) {
      throw new Error("Connect account webhook configuration is incomplete");
    }
    const rawBody = await readRawBody(context.request);
    const stripe = createStripePayoutClient(config.stripeSecretKey);
    const outcome = await handleConnectAccountWebhook({
      rawBody,
      signature,
      dependencies: {
        stripe,
    ...
    console.info("[connect-account-webhook] handled", {
    ...
    console.error("[connect-account-webhook] rejected", { code });
    return json(
      {
        ok: false,
